...
import { sortAccountsByStatus, getUniqueGameIds } from "@/utils/admin.util";
...
export default function AccountsPage() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, gameId, accountType, priceRange]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (gameId && gameId !== "all") {
          queryParams.gameCategoryId = Number(gameId);
        }

        // Add account type filter
        if (accountType && accountType !== "all") {
          queryParams.typeAccount = accountType;
        }
        ...
        const [categoriesRes, accountsRes] = await Promise.all([
          listGameCategories(),
          listGameAccounts(queryParams),
        ]);

        setGames(categoriesRes.data || []);
        // Access data correctly from normalized response
        const accountsData = accountsRes.data?.data || [];
        const pagination = accountsRes.data?.pagination;

        setAccounts(accountsData);
        setTotalAccounts(pagination?.total || accountsData.length);
    ...
    fetchData();
  }, [currentPage, searchQuery, gameId, accountType, priceRange]);
  ...
  // Sort accounts by status (already filtered by server)
  const sortedAccounts = sortAccountsByStatus(accounts);
