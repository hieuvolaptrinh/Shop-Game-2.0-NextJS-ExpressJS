...
export default function UserDetailPage({
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const [userRes] = await Promise.all([getUserById(userId)]);
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const userId = parseInt(id);
        const ordersRes = await getOrdersByUserId(userId, {
          page: ordersPage,
          limit: itemsPerPage,
        });
        ...
        const ordersData =
          ordersResAny?.data?.data ??
          ordersResAny?.data?.orders ??
          (Array.isArray(ordersResAny?.data) ? ordersResAny.data : []);

        const pagination = ordersResAny?.data?.pagination;

        setUserOrders(ordersData);
        setOrdersTotalItems(pagination?.total || ordersData.length);
  ...
  // Fetch recharges with pagination
  useEffect(() => {
    const fetchRecharges = async () => {
      try {
        const userId = parseInt(id);
        const rechargesRes = await getUserDepositRequests(userId, {
          page: rechargesPage,
          limit: itemsPerPage,
        });

        const rechargesResAny = rechargesRes as any;
        const rechargesData =
          rechargesResAny?.data?.data ??
          (Array.isArray(rechargesResAny?.data) ? rechargesResAny.data : []);

        const pagination = rechargesResAny?.data?.pagination;

        setUserRecharges(rechargesData);
        setRechargesTotalItems(pagination?.total || rechargesData.length);
        setRechargesTotalPages(pagination?.totalPages || 1);
      } catch (error) {
        ...
        <div className="bg-[#1a1a1a] border border-[#3f9ced]/20 rounded overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div className="flex gap-2 mb-6 border-b border-[#3f9ced]/20">
              ...
              >
                Đơn hàng đã thực hiện ({ordersTotalItems})
              </button>
              <button
              ...
              >
                Lịch sử nạp ({rechargesTotalItems})
              </button>
            </div>
            ...
            {activeTab === "recharges" && (
              <UserRechargesTable
                ...
                totalItems={rechargesTotalItems}
                itemsPerPage={itemsPerPage}
                onPageChange={setRechargesPage}
              />
            )}
          </div>
        </div>
