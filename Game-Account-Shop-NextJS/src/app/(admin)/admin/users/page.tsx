  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        ...
        const usersData =
          responseAny.data?.data || responseAny.data?.users || [];

        // Normalize data: convert userId to id, money to balance
        const normalizedUsers = usersData.map((user: any) => ({
          id: user.userId?.toString() || user.id?.toString(),
          email: user.email,
          role: user.role,
          balance: Number(user.money || user.balance || 0),
          createdAt: user.createdAt,
          lastLogin: user.lastLogin,
        }));

        setUsers(normalizedUsers);
        setTotalUsers(responseAny.data?.total || usersData.length);
        setTotalPages(responseAny.data?.totalPages || 1);
      } catch (error: any) {
